...
const Redeem = () => {
  ...
  useEffect(() => {
    if (!context.privateLink || !context.chainId || !context.provider) {
      console.log("context.privateLink", context.privateLink);
    ...
    populateEvents(context.privateLink, context.chainId, context.provider)
      .then((events) => setEvents(events))
      .catch(console.log)
      .finally(() => setEventsLoading(false));
  }, [context.privateLink, context.chainId, context.provider]);
  ...
  const redeem = async () => {
    if (!context.privateLink || !context.chainId) {
      console.log("redeem() context.privateLink", context.privateLink);
    ...
    const transaction = await context.privateLink.withdraw(
      proof.a,
      proof.b,
      proof.c,
      nullifierHash,
    ...
    // const transaction = await context.privateLink.withdraw([proof.pi_a[0], proof.pi_a[1]], [[proof.pi_b[0][1], proof.pi_b[0][0]], [proof.pi_b[1][1], proof.pi_b[1][0]]] , [proof.pi_c[0], proof.pi_c[1]], nullifierHash, address, root);
